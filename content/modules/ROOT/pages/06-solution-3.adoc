...
We will demonstrate object detection using YOLO (You Only Look Once) which is a popular object detection and image segmentation model,
...
We will start with Data Preparation following by Training and Model Serving.
...
You should not have any jupyterhub notebook running for this user. Just incase you have a running notebook then you can stop.
* If you already have the previous notebook server running,
...
* Once you stop the notebook server, you should switch over to terminal tab on your showroom browser window
and login to the cluster using the below command.
...
Note: minio is a very simple way of implemented S3 storage but its not recommended for production use.
For production use, you should leverage Container Storage like OpenShift Data Foundation.

:g_uid: {GUID}
:sandbox_zoneid: {sandbox_name}

* Run this section in the terminal window (showroom terminal tab) to update minio's setup-bucket-configmap.
****
...
* Post deployment of this script, you should see this in your OpenShift AI dashboard. (Data Science Projects)
...
is all automated using the above script.
...
In this case , you can open minio dashboard, and upload this file to the s3 bucket.
We have already done this step in automated script setup.sh earlier and have yolov5.onnx model in the s3 bucket.
...
We have already completed this step in previous setup.sh automated script.
...
Optional Section if you have time, else read through this section for information on how model training works.

Following is explanation of how Model Training is done step by step including re-training of the model.
Note that we are not using GPU's so you can go till the last step in this section but the model training will not work (its going to be too slow to see that its working) as we do not have GPU's,
but you will get the idea. If you want to run this on GPU's, you can start a CI with `Base RHODS with NVIDIA on AWS` from demo.redhat.com, clone this repo and you should be good to go.
