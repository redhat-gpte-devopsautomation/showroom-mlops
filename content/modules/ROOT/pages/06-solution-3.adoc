...
and login to the cluster using the below command.
 ...
 oc login --insecure-skip-tls-verify=false -u {openshift_cluster_admin_username} -p {openshift_cluster_admin_password} {openshift_api_server_url}
...
Following is explanation of how its done step by step including re-training of the model..
...
For this first step (This step is already completed when you ran setup.sh script earlier):
