= MLOps (GitOps)
:navtitle: 3: Syndergy - MLOps with GitOps
:numbered:
...
* link:#workshop-environment[Workshop Environment.]
** link:#for-red-hat-employees[For Red Hat Employees:]
** link:#for-the-general-public[For the general public:]
* link:#accessing-and-testing-your-deployed-application[Accessing and
testing your deployed Application]
* link:#retraining-the-model[Retraining the model]
** link:#logging-into-rhods[Logging into RHODS]
** link:#git-clone-the-arc-model-project[Git clone the arc-model
project]
** link:#retrain-the-model[Retrain the model]
** link:#publish-the-changes[Publish the changes]
* link:#reviewing-the-openshift-pipeline[Reviewing the OpenShift
Pipeline]
** link:#reviewing-the-pipeline-run[Reviewing the pipeline run]
** link:#retrain-the-model-again[Retrain the model (again).]
** link:#watch-the-build[Watch the build.]
* link:#reviewing-argocd-and-gitops[Reviewing ArgoCD and GitOps]
** link:#connecting-to-openshift-gitops[Connecting to OpenShift GitOps]
** link:#attempting-a-manual-change-in-openshift[Attempting a manual
change in OpenShift]
** link:#updating-things-the-gitops-way[Updating things the GitOps way]
* link:#wrap-up-of-hands-on[Wrap up of Hands-On]

=== Workshop Environment.

==== For Red Hat Employees:

If you are a Red Hat employee, you can reserve the workshop environment
using the https://rhpds.redhat.com/[RHPDS] system.

image:instructions/rhpds.png[instructions/rhpds]

==== For the general public:

If you do not work for Red Hat, an environment will be provided for you
as part of the workshop.

Follow the link found below to claim an environment - simply enter your
email (any email belonging to you) and the password will be given during
the presentation.

https://demo.redhat.com/workshop/h7bha9

After logging in, you’ll be redirected to a site containing all the URLs
and logins you’ll need to complete the workshop. That page should look
like this:

image:instructions/logins-screen.png[instructions/logins-screen]
...
[arabic]
. Follow the URL marked as `Application url'
image:instructions/application-url.png[instructions/application-url]
...
In this section, we will: * log into RHODS * Clone the project from your
on-cluster gitea instance * Retrain the model with fresher data *
Publish our changes

==== Logging into RHODS

* Locate the RHODS-Dashboard URL. (It starts with
...
information here:
+
image:instructions/gittea-url.png[instructions/gittea-url]
...
** image:instructions/copy-git-url.png[instructions/copy-git-url]
* Navigate back to your JupyterLab tab
* Click on the Git Icon (fourth of the 6 big icons on the left)
* Click on the *Clone a repository* button
** image:instructions/clone-repo.png[instructions/clone-repo]
...
* Open the Notebook called `5_discount_model.ipynb` from the RHODS file
system tab
...
All Cells*
** image:instructions/csv-restart-run-all.png[instructions/csv-restart-run-all]
...
image:instructions/select-administrator.png[instructions/select-administrator]
. Navigate to *Pipelines* , then *Pipelines* (yes, again), and then go
to *PipelineRuns*
image:instructions/select-pipelines.png[instructions/select-pipelines]
...
image:instructions/commit-msg.png[instructions/commit-msg]
