...
... Earlier when you login to GLBOEX App UI as a customer and commented on a product (for e.g. we commented on cothing category - T-shirt above), it gets submitted to the *globex.reviews* kafka topic, which then gets picked up by the Intelligent App *aiml-moderate-reviews*
...
You go to your topology view as a developer and ensure that you are selecting sentiment-analysis project. If you review carefullly, you can see that all the aiml services and the persist review service are all knative serverless services.
...
In summary, the Product Manager is able to get a good understanding of customer sentiments. PM also have a dashboard to view realtime and historical sentiment trend to take timely data driven decisions.
Now in this scenario we see that there are more negative reviews and the KPI of achieving 60% or more positive sentiments for a given category is not achieved. What we will see next is that PM along with Marketing team comes up with a strategy to address this challenge.
